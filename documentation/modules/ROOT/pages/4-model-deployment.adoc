...
これをコピーしてください。ノートブックの最初のセルにある `replace_me` プレースホルダーを置き換えるのに数秒で必要になります。
...
* 準備ができたら、 **Inference endpoint** 列の下にある `Internal Service` リンクを選択し、grpcURL 値をメモします。後で必要になります。これは`grpc://modelmesh-serving.%USERID%-data-science-project:8033` になっているはずです。
